...
commit: 426dbd2d753755ed18c8347984731298b8dd78ce
updated: 2026-01-17T19:15:00+00:00
---
...
| `DEFAULT_STREAM_CHUNK_SEC` | `8` | Default chunk length when `--stream` is enabled |
| `DEFAULT_BATCH_SIZE` | `12` | Batch size for inference |
| `DEFAULT_VAD` | `False` | Enable VAD during stabilization by default |
| `DEFAULT_STABILIZE` | `False` | Enable stable-ts stabilization by default |
| `DEFAULT_DEMUCS` | `False` | Enable Demucs by default |
| `DEFAULT_WORD_TIMESTAMPS` | `False` | Enable word timestamps by default |
| `PARAKEET_MODEL_NAME` | `nvidia/parakeet-tdt-0.6b-v3` | Default ASR model |
| `FORCE_FFMPEG` | `1` | Prefer FFmpeg decoding before `soundfile` |
| `MIN_CPS` | `10` | Minimum characters per second |
| `MAX_CPS` | `22` | Maximum characters per second |
...
| Area | Choice |
| -- | -- |
| GPU runtime | ROCm 6.4.2 (wheel source; host bind-mounts in Docker) |
| Deep-learning stack | PyTorch 2.7.1 ROCm wheels + torchaudio 2.7.1 |
| Model hub | Hugging Face `nvidia/parakeet-tdt-0.6b-v3` (default), `nvidia/parakeet-tdt-0.6b-v2` |
| Framework | NVIDIA NeMo 2.4.x (`nemo-toolkit[asr]>=2.4,<2.5`) |
| Timestamp refinement | stable-ts-whisperless (`stable_whisper` module) |
| Package manager | PDM 2.15+ (lockfile + requirements exports) |
