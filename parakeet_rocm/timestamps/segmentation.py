...
def _split_at_clause_boundaries(sentence: list[Word]) -> list[list[Word]]:
    ...
    The function prefers to break at clause-ending punctuation when that
    yields readable, limit-respecting chunks. If no suitable boundary exists,
    it falls back to a greedy splitting strategy.

    Parameters:
        sentence (list[Word]): Sequence of ``Word`` objects representing the
            sentence to split.

    Returns:
        list[list[Word]]: Sentence chunks where each chunk aims to satisfy
            the module's length, duration, and CPS limits.
    """
    if not sentence:
        return []
...
def _greedy_split_fallback(sentence: list[Word]) -> list[list[Word]]:
    """Split a sentence into chunks at word boundaries.

    This is a greedy fallback used when no suitable clause boundaries are
    found. Words are accumulated into a chunk until adding the next word
    would violate character, duration, or CPS limits, then a new chunk is
    started. If a single word itself exceeds limits, it is returned as its
    own chunk.

    Parameters:
        sentence (list[Word]): Words representing a single sentence.

    Returns:
        list[list[Word]]: Word chunks; each chunk is a list of ``Word``
            objects that respect the module's display and timing constraints
            where possible.
    """
    if not sentence:
        return []
...
def _eliminate_orphan_words(sentences: list[list[Word]]) -> list[list[Word]]:
    """Merge very short sentence fragments into adjacent sentences.

    An orphan is a sentence whose plain text is shorter than 15 characters,
    has two or fewer words, or contains a single token. The function attempts
    to merge an orphan with the previous sentence (or with the next sentence
    if the orphan is the first). A merge is applied only if the combined
    sentence satisfies content limits as determined by ``_respect_limits``.
    If a valid merge cannot be made, the orphan is kept as its own sentence.

    Parameters:
        sentences (list[list[Word]]): Sentences where each sentence is a list
            of ``Word`` objects.

    Returns:
        list[list[Word]]: Sentences with orphan fragments merged where
            appropriate.
    """
    if len(sentences) <= 1:
        return sentences
...
def _respect_limits(words: list[Word], *, soft: bool = False) -> bool:
    """Check whether a word sequence respects length and timing limits.

    Parameters:
        words (list[Word]): Ordered word tokens forming the candidate
            segment.
        soft (bool, optional): When True, apply the softer character limit
            to allow slight overflows during merges. Defaults to False.

    Returns:
        bool: True if the sequence meets all limits, False otherwise.
    """
...
def _sentence_chunks(words: list[Word]) -> list[list[Word]]:
    """Split words into sentence-like chunks.

    The function uses strong sentence punctuation together with
    clause-aware merging. Tokens ending with ".", "!", or "?" are treated
    as primary sentence boundaries. A short trailing fragment is merged into
    the previous sentence when the combined text length and duration remain
    within block and segment limits.

    Returns:
        list[list[Word]]: Sentence chunks where each chunk is a list of
            ``Word`` objects representing one sentence-like unit.
    """
    if not words:
        return []
    ...
    for i, word in enumerate(words):
        ...
        if word.word.rstrip().endswith((".", "!", "?")):
            # Only create a sentence if it has more than a lone punctuation token
...
def segment_words(words: list[Word]) -> list[Segment]:
    """Split word tokens into subtitle segments.

    The function groups words into sentence-like chunks, splits overly long
    chunks at clause boundaries (or by a greedy fallback), merges orphans and
    short adjacent chunks when limits allow, applies minimum display padding,
    and adjusts timestamps so segments meet character, duration, and
    characters-per-second constraints.

    Returns:
        list[Segment]: Subtitle ``Segment`` objects with formatted text,
            associated words, start time, and end time.
    """
    if not words:
        return []
