...
class JobManager:
    ...
    def __init__(
        ...
        self.benchmark_enabled = (
            enable_benchmarks if enable_benchmarks is not None else BENCHMARK_PERSISTENCE_ENABLED
        )
    ...
    def run_job(self, job_id: str, progress_callback: callable | None = None) -> TranscriptionJob:
        ...
        try:
            ...
            # Print full traceback to stderr (logging may be disabled by quiet mode)
            import sys
            import traceback

            print(
                f"\n[ERROR] Transcription job {job_id[:8]} failed: {e}",
                file=sys.stderr,
            )
