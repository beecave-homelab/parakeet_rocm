...
def to_txt(result: AlignedResult, **kwargs: object) -> str:
    ...
    Parameters:
        result (AlignedResult): The aligned result containing ordered
            segments whose ``text`` values will be concatenated.
        **kwargs: Additional keyword arguments (ignored for plain text output).

    Returns:
        str: A single string of all segment texts joined with a single
            space. Returns an empty string if there are no segments.
    """
