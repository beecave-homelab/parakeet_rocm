...
Notes:
    ...
    integrated easily. It focuses on a few intuitive checks (overlaps, hyphen
    spacing, line lengths, CPS bounds) that correlate well with readable subtitles.
"""
...
def compute_srt_quality(
    ...
    line_length_violations = sum(
        1 for ln in text_lines if len(ln) > constant.MAX_LINE_CHARS
    )
    ...
    for index, seg in enumerate(segments or []):
        try:
            ...
            if dur < constant.MIN_SEGMENT_DURATION_SEC:
                boundary_counts["too_short"] += 1
            elif dur > constant.MAX_SEGMENT_DURATION_SEC:
                boundary_counts["too_long"] += 1
            else:
                boundary_counts["within_range"] += 1
            if constant.MIN_CPS <= cps <= constant.MAX_CPS:
                cps_ok_count += 1
                cps_histogram["within_range"] += 1
            elif cps < constant.MIN_CPS:
...
def _collect_line_length_offenders(text_lines: list[str]) -> list[dict[str, Any]]:
    ...
    max_chars = constant.MAX_LINE_CHARS
